// Teensy 4.1 and Ragazza
// Copyright (C) 2023 https://www.roboticboat.uk
// 5b634767-b207-4a56-9354-e1bcb0eac7c9
//
...
void serialEvent1()
{
  // Process new bytes from GPS module
  gps.listen();
}
...
void loop()
{
  ...
  {  
    ...
    sprintf(sdbuffer, "Ragazza03,$TMR,%lu,0,$RC,%d,%d,%d,$CMP,%.0f,%d,%d,%c%c%c%c%c%c%c%c,$ACC,%f,%f,%f,$GYR,%f,%f,%f,$GPS,%.0f,%.0f,%.8f,%c,%.8f,%c,%.1f,%d,%d,%.2f,%.2f,%c,$WND,%d,$HALL,%d,%d", 
                      mytime,ch1,ch2,ch3,
    ...
    Serial8.print("$");
    Serial8.print(WiFiChecksumMessage);
    Serial8.print("*");
    sprintf(strNumber,"%02X",MakeChecksum(WiFiChecksumMessage));
    Serial8.print(strNumber); 
