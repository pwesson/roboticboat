// Teensy 4.1 and Seawind
// Copyright (C) 2026
// a623e847-5abe-4c50-8ca8-21630d09fc91
//
...
void updateLogging() {
    ...
    // Update the User
    sprintf(sdbuffer, "Seawind33,$TMR,%lu,$RC,%d,%d,%d,$CMP,%.0f,%d,%d,%c%c%c%c%c%c%c%c,$ACC,%.3f,%.3f,%.3f,$GYR,%.3f,%.3f,%.3f,$GPS,%.0f,%.0f,%.8f,%c,%.8f,%c,%.1f,%d,%d,%.2f,%.2f,%c,%.0f,$HALL,%d,%d,$BNO,%.0f,%.0f,%.0f", 
                      millis(),ch1,ch2,ch3,
                      ...
                      outputV1,outputV2,
                      bno086_roll, bno086_pitch, bno086_yaw);
   ...
   // Create WiFi Message
    sprintf(WiFiChecksumMessage,"MSG,%lu,%.0f,%d,%d,%.8f,%.8f,%d,%d,%d,%d,%d",millis(),BoatHeading,cmps14.pitch,cmps14.roll,gps.latitude,gps.longitude,gps.numsatelites,ch1,ch2,outputV1,outputV2);
    Serial3.print("$");
