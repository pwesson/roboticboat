// Teensy 4.1 and MicroMagic
// Copyright (C) 2026
// aa773bec-50fb-4313-a4f5-dc4001c16488
//
...
void readIMU() {
    ...
    if (q.status == 3 && q.real > 0.01f && q.real <= 1.0f) {
        ...
        // The IMU is not pointing to North when it starts.
        // Lets compare with CMPS14 compass and work out the difference
        cr.Update(bno086_yaw, BoatHeading);
        bno086_yaw2 = cr.PredictY(bno086_yaw);
    }
}
...
void updateLogging() {
    ...
    // Update the User
    sprintf(sdbuffer, "MicroMagic65,$TMR,%lu,$RC,%d,%d,%d,$CMP,%.0f,%d,%d,%c%c%c%c%c%c%c%c,$ACC,%.3f,%.3f,%.3f,$GYR,%.3f,%.3f,%.3f,$GPS,%.0f,%.0f,%.8f,%c,%.8f,%c,%.1f,%d,%d,%.2f,%.2f,%c,%.0f,$HALL,%d,$BNO,%.0f,%.0f,%.0f,%.1f", 
                      millis(),ch1,ch2,ch3,
                      ...
                      outputV1,
                      bno086_roll, bno086_pitch, bno086_yaw, bno086_yaw2);
